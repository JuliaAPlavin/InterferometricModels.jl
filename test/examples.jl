...
# ╔═╡ fdbd4c4e-a438-49d4-a814-4fc7a95dfe24
using Pkg

# ╔═╡ a80f4446-a49d-45e8-be84-5676dc172ffc
...
using PlutoUIExtra; using PlutoUIExtra: Slider
...
image(comp_img, colormap=Reverse(:inferno))

# ╔═╡ 12c15b34-a8fa-450c-a34b-1e26b07e9ded
image(comp, colormap=Reverse(:inferno))
...
let
	...
	vlines!([-1, 1] * fwhm_max(comp)/2, linestyle=:dash)
	vlines!([-1, 1] * fwhm_min(comp)/2, linestyle=:dash)
	fig
end
...
# ╔═╡ f367ef82-2d36-408d-a57f-568ca543e48d
image(comp_convolved, colormap=Reverse(:inferno), xyintervals=0±15)
...
# ╔═╡ 7d482435-ef34-43ab-9707-4d21b6108031
let
	fig, _, _ = poly(mod)
	scatter!(mod, color=:black, marker=:cross, alpha=0.5)
	fig
end
...
# ╔═╡ f4a43a9c-9ba1-4d78-bdbc-78bc57aa9157
let
	fig, ax, plt = image(mod_img, colormap=Reverse(:inferno), colorscale=SymLog(1e-3))
	Colorbar(fig[1,2], plt, label=unit(eltype(mod_img)) |> string)
	fig
end
...
# ╔═╡ 3cb7b93d-e521-4df6-ae29-29165f5c7009
let
	fig, ax, plt = image(convolve(ustrip(mod), b), xyintervals=0±15, colormap=Reverse(:inferno), colorscale=SymLog(1e-3))
...
# ╔═╡ 08a29695-9beb-415c-9fa7-860758eebb85
scatter(norm.(uvs), abs.(ustrip.(vises)), markersize=5, color=:black, axis=(xlabel="UV distance (λ)", ylabel="Visibility amplitude (Jy)"))

# ╔═╡ 5dbfa878-ebd2-4112-bc98-0f2921c2c7f5
md"""
It's also convenient to compare contributions of each model component at different UV distances. `InterferometricModels` provides the `visibility_envelope` function: it calculates the range of possible visibility amplitudes and phases of a model at a fixed UV distance:
"""

# ╔═╡ e6929ddf-3236-4627-be5e-d5db9c2a3bc0
visibility_envelope(abs, components(mod)[1], 1e9)

# ╔═╡ 5bb603f7-1af0-4e53-b21a-63a599e2badf
visibility_envelope(abs, mod, 1e9)
...
# ╔═╡ bad49d14-c0ab-480f-8589-e1f788178ac7
begin
	uvdist = Observable(1e7)
	uvang = Observable(30u"°")
end;

# ╔═╡ 3fec21ab-9497-4603-bd61-0ca398e5476d
uv = @lift $uvdist .* SVector(sincos($uvang)) |> reverse;

# ╔═╡ 3ae7bb8d-e8e0-4b3a-a625-04ab72aba2f4
md"""
UV location is controlled with two slides below, setting distance and angle.
"""
...
md"""
UV dist: $(@bind uvdist_ Slider(0:2e6:1e8; default=uvdist[], show_value=true))

UV angle: $(@bind uvang_ Slider((-90:5:90)u"°"; default=uvang[], show_value=true))
"""

# ╔═╡ eb767d44-1917-4437-b1db-cbf1e3a24216
uvdist[] = uvdist_;

# ╔═╡ 1d4ce1ba-f464-44fe-b9ae-d5845d0bce48
uvang[] = uvang_;

# ╔═╡ f9ade4e4-0d7e-41d1-8a64-c2208deac706
let
	fig = Figure()
	
	band(fig[1,1], RadPlot(0..3e8, model=emod), color=(:black, 0.15))
	lines!(RadPlot(0..3e8, model=emod), color=(:black, 0.8))
	scatter!((@lift RadPlot([(;uv=$uv)], model=emod)), color=:orange, markersize=30)

	fig
end

# ╔═╡ 6bebcbc4-47f1-4759-9fc9-8a9c7941427b
let
	fig = Figure()
	ax = Axis(fig[1,1], aspect=DataAspect(), limits=(-0.1..2, 0±1), xlabel="Re (Jy)", ylabel="Im (Jy)")

	poly!(Circle(Point2(0., 0), ustrip(flux(emod))), color=(:black, 0), strokecolor=(:black), strokewidth=1)
	text!((1.7, -0.8), text="Total flux", align=(:right, :center), color=:black)
	
	map(0:length(components(emod))) do n
		curvis = @lift @p sum(visibility(components(emod)[i], $uv) for i in 1:n; init=0u"Jy") |> ustrip(u"Jy", __)
		prevvis = @lift @p sum(visibility(components(emod)[i], $uv) for i in 1:(n-1); init=0u"Jy") |> ustrip(u"Jy", __)
		n > 0 && lines!((@lift reim.([$prevvis, $curvis])), label=n == 0 ? "Total" : "Comp #$n", linewidth=3)
		n == length(components(emod)) && lines!((@lift [(0, 0), reim($curvis)]); color=:black, linestyle=:dot, label="Total")
		scatter!((@lift reim($curvis)); color=n == 0 ? :black : Cycled(n), markersize=10, label=n == 0 ? "Total" : "Comp #$n")
	end
...
manifest_format = "2.0"
project_hash = "6171c37fea35a7b3b935fe508e61b9463a5155bf"
...
git-tree-sha1 = "55def4893dce3df207b5063820f7448a0efd483a"
...
git-tree-sha1 = "0000eb323e2b4b2f76bda0e4cd0ddbe26bb8276d"
...
git-tree-sha1 = "c1b1d91855cb5f21d163933b62aab490f5f9fcaf"
...
version = "0.1.12"
weakdeps = ["Accessors", "AxisKeys", "Dictionaries"]
...
git-tree-sha1 = "5fd39225a780d51c8389bd2482704cf0c4907ab8"
...
[[deps.VLBIPlots]]
deps = ["Accessors", "AxisKeysExtra", "DataManipulation", "InterferometricModels", "IntervalSets", "InverseFunctions", "LinearAlgebra", "MakieExtra", "RectiGrids", "StaticArrays", "Statistics", "Unitful"]
git-tree-sha1 = "0fdbcaf881c85198088a2ee34f5141d459e5e9c6"
...
git-tree-sha1 = "90223a7f111453b48c5ebf3c3541b6d657c01149"
